...
def build_model():
    ...
    model = make_pipeline(preprocessor, RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1))
    return model
...
def train_and_save():
    ...
    joblib.dump({"model": model, "name": "RandomForest"}, MODEL_PATH)
...
def sanity_checks(model):
    ...
    court = pd.DataFrame([test_cases[1]])
