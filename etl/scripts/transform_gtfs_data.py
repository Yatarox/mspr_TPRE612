...
def build_stop_country_map(stops_df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    Utilise des bounding boxes pour performance (pas besoin de GeoJSON lourd).
    """
...
def _process_trips_chunk(trips_chunk: pd.DataFrame, first, last, stops_name,
    ...
    for idx, tr in enumerate(trips_chunk.iterrows()):
        ...
        all_rows.append({
            ...
            "origin_country": origin_country,  # Peut être None
            "destination_stop_name": destination_stop_name,
            "destination_country": destination_country,  # Peut être None
            "departure_time": dep,
            "arrival_time": arr,
...
def build_trips_summary_for_dataset(
    ...
    # Accumulate all rows before writing
    all_rows: List[Dict] = []
