...
def get_latest_gtfs_from_api(base_url: str) -> Optional[Dict[str, str]]:
    try:
        ...
        if format_type != "GTFS":
            logger.warning(f"Latest file is not GTFS format (found: {format_type})")
...
def check_if_already_extracted(extract_dir: str, filename: str, file_hash: Optional[str] = None) -> bool:
    ...
    gtfs_files = ["agency.txt", "routes.txt", "trips.txt", "stops.txt", "stop_times.txt"]
    ...
    if file_hash:
        try:
            ...
            if stored_hash != file_hash:
                logger.info(f"Dataset {dataset_name} has different version (hash mismatch)")
                return False
        except Exception as e:
...
def download_file(url: str, output_path: str, force_download: bool = False) -> Optional[str]:
    ...
    if not force_download and os.path.exists(output_path):
        logger.info(f"  → File already exists: {os.path.basename(output_path)}")
    ...
    try:
        ...
        logger.info(f"  ✓ Downloaded: {os.path.basename(output_path)} ({downloaded_size / 1024 / 1024:.2f} MB)")
...
def extract_zip(zip_path: str, extract_path: str) -> bool:
    ...
    try:
        ...
        logger.info(f"  ✓ Extracted successfully ({len(found_files)} GTFS files found)")
        return True
...
def download_and_extract_gtfs(
    ...
    for idx, (url, info) in enumerate(download_map.items(), 1):
        ...
        if not force_download and check_if_already_extracted(extract_dir, filename):
            logger.info(f"✓ Dataset '{dataset_name}' already extracted, skipping")
...
def clean_old_downloads(download_dir: str, keep_latest: int = 2):
    ...
    if len(zip_files) <= keep_latest:
        logger.info(f"No cleanup needed ({len(zip_files)} files, keeping {keep_latest})")
        return
...
def download_and_unzip(
    ...
    """
    logger.warning("Using deprecated download_and_unzip, use download_and_extract_gtfs instead")
    return download_and_extract_gtfs(data, download_dir, extract_dir, force_download)
...
def download_from_direct_urls(
    ...
    logger.info(f"\n✓ Processed {len(dataset_sources)} datasets from direct URLs")
    return dataset_sources
